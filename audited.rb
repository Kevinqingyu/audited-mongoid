...
module Audited
  ...
  private
    ...
    def audit_create
      write_audit(auditable_id: self.id, auditable_type: self.class.to_s, action: 'create', audited_changes: audited_attributes)
    end

    def audit_update
      if (changes = audited_changes).present?
        write_audit(auditable_id: self.id, auditable_type: self.class.to_s, action: 'update', audited_changes: changes)
      end
    end

    def audit_destroy
      unless self.new_record?
        write_audit(auditable_id: self.id, auditable_type: self.class.to_s, action: 'destroy', audited_changes: audited_attributes)
      end
    end
